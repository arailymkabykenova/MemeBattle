...
@router.get("/azure/debug", response_model=Dict[str, Any])
async def debug_azure_storage(
    db: AsyncSession = Depends(get_db)
):
    """
    🔍 ДИАГНОСТИКА: Показывает все файлы в Azure контейнере.
    
    Используется для отладки - показывает какие папки и файлы видит система.
    """
    try:
        return await azure_service.debug_list_all_blobs()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Ошибка диагностики Azure: {str(e)}")
...
@router.post("/azure/test-load-all", response_model=Dict[str, Any])
async def test_load_all_cards_from_azure(
    db: AsyncSession = Depends(get_db)
):
    """
    🧪 ТЕСТ: Загружает все карты из Azure без аутентификации.
    
    Временный эндпоинт для тестирования системы загрузки карт.
    """
    card_service = CardService(db)
    try:
        return await card_service.load_all_cards_from_azure()
...
@router.get("/user/{user_id}/cards", response_model=Dict[str, Any])
async def get_user_cards(
    ...
    """
    Получает карты конкретного пользователя (для просмотра коллекций).
